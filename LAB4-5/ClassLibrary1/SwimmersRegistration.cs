...
namespace ClassLibrary1
{
    public static class SwimmersRegistration
    {
        ...
        private static void ProcessSwimmerRecord(string swimmerRecord, string clubName)
        {
            ...
            byte ageGroup = ageToAgeGroup((byte)(DateTime.Now.Year - birthday.Year));
        ...
        {
            meet = m;
            string[] registrationFiles = Directory.GetFiles(DefaultRegistrationFolder, "*.txt");
            foreach (string filePath in registrationFiles)
            {
                ProcessClubRegistrationFile(filePath);
            }
        }
